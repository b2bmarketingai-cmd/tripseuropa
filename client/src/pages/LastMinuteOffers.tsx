...
const LAST_MINUTE_OFFERS = [
  ...
  },
  {
    ...
    badge: { es: "Mas Vendido", en: "Best Seller" }
  },
  {
  ...
  },
  {
    ...
    badge: { es: "Venta Flash", en: "Flash Sale" }
  },
  {
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <Badge className="bg-accent/20 text-accent border-accent/30" data-testid="badge-lastminute">
              {language === "es" ? "Ofertas Exclusivas" : "Exclusive Deals"}
            </Badge>
            <Flame className="w-6 h-6 text-accent" />
          </div>
          
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold text-white mb-6" data-testid="text-lastminute-title">
            <span className="text-accent">{language === "es" ? "Ofertas de Ultima Hora" : "Last-Minute Deals"}</span>
          </h1>
          ...
          <div className="flex flex-wrap justify-center gap-4 text-white/80 text-sm">
            <div className="flex items-center gap-2">
              ...
              <span>{language === "es" ? "Hasta 60% de descuento" : "Up to 60% off"}</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-accent" />
              <span>{language === "es" ? "Vuelos incluidos" : "Flights included"}</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-accent" />
              <span>{language === "es" ? "Hoteles 4-5 estrellas" : "4-5 star hotels"}</span>
            </div>
            <div className="flex items-center gap-2">
              <CheckCircle className="w-4 h-4 text-accent" />
              <span>{language === "es" ? "Guias expertos" : "Expert guides"}</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-2xl md:text-3xl font-display font-bold mb-6 text-foreground">
              <span className="text-accent">{language === "es" ? "Paquetes de Vacaciones de Ultima Hora" : "Last-Minute Vacation Packages"}</span>
            </h2>
                ...
                <Card className="group overflow-visible cursor-pointer hover-elevate h-full" data-testid={`card-offer-${offer.id}`}>
                  <div className="relative h-56 overflow-hidden rounded-t-lg">
                    ...
                    <div className="absolute bottom-3 left-3 right-3 text-white">
                      <h3 className="text-lg font-display font-bold text-accent leading-tight">{offer.title[lang]}</h3>
                      <div className="flex items-center gap-2 text-white/90 text-sm mt-1">
                        <MapPin className="w-3 h-3" />
                        <span>{offer.country[lang]}</span>
                        <span className="mx-1">|</span>
                        <Clock className="w-3 h-3" />
                        <span>{offer.days} {language === "es" ? "dias" : "days"}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground mb-4">
                      ...
                      <span>{language === "es" ? "Proximas salidas:" : "Next departures:"}</span>
                    ...
                    <div className="flex items-end justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">USD {language === "es" ? "por persona" : "per person"}</p>
                      </div>
                      <Button size="sm" data-testid={`button-offer-${offer.id}`}>
                        {language === "es" ? "Ver oferta" : "View deal"}
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-2xl md:text-3xl font-display font-bold mb-6 text-center">
              <span className="text-accent">{language === "es" ? "Ofertas de Viaje de Ultima Hora" : "Last-Minute Travel Deals"}</span>
            </h2>
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-2xl mx-auto text-center">
            <h2 className="text-2xl md:text-3xl font-display font-bold text-white mb-4">
              {language === "es" ? "Suscribete y Unete a TripsEuropa GO" : "Subscribe and Join TripsEuropa GO"}
            </h2>
            <div className="flex flex-wrap justify-center gap-4 text-white/80 text-sm mb-6">
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-accent" />
                <span>{language === "es" ? "Las mejores ofertas" : "The best travel deals"}</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-accent" />
                <span>{language === "es" ? "Promociones exclusivas" : "Exclusive promotions"}</span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-accent" />
                <span>{language === "es" ? "Tips de viaje de expertos" : "Expert travel tips"}</span>
              </div>
            </div>
        ...
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 text-center md:text-left">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-display font-bold text-lg">
                  {language === "es" ? "Mejor Precio Garantizado" : "Best Price Guaranteed"}
                </h3>
            ...
            <a href="tel:+34611105448" className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-display font-bold text-lg">
                  {language === "es" ? "Reserva por Telefono" : "Book by Phone"}
                </h3>
